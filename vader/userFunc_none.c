...
userPostTimestep(const double t, const double dt,
  ...
  for (int i = grd->nr-1; i >= 0; i--) {
    ...
    if (grd->r_g[i+1] < 1.32265361 * cm_in_AU && peak == grd->nr-1) {
      peak = i+1;
    }
  }
  if (edge == grd->nr-1) { edge -= 1; }

  // Internal photoevaporation algorithm
  // Surface density flux is max at ~1.3 AU, independent of other parameters. Mass flux is R times that; 
  // we assume that all mass is removed beyond this peak. We then seek to 'flatten' the surface density
  // beyond that peak until enough mass has been removed.
  while (dM_inner_out < dM_inner) {
    // Search for first cell beyond peak having lower surface density than peak
    for (j = peak+1; j < grd->nr-1; j++) {
      if (col[j] < col[peak]) {
        break;
      }
    }
    // If there is no cell beyond peak with a lower column density, stop evaporating (otherwise, stuck in loop)
    if (col[j] == col[peak]) { break; }

    // For every cell between peak and the cell lower than peak, remove mass up to that lower level
    for (int i = peak; i < j+1; i++) {
      //dM = (col[i] - max())*grd->area[i];  ((col[j] > C0) ? col[i] - col[j] : col[i] - C0)*grd->area[i];
      //dM = col[j] > 1.1*C0 ? (col[i] - col[j])*grd->area[i] : col[i]*0.5*grd->area[i];
      //dM = (col[i] - col[j])*grd->area[i];
      dM = col[j] < col[i]*0.5 ? col[i]*0.5*grd->area[i] : (col[i] - col[j])*grd->area[i];
